...
import com.o19s.solr.swan.highlight.spanawarevectorhighlight.SAFastVectorHighlighter;
import com.o19s.solr.swan.highlight.spanawarevectorhighlight.SAFieldQuery;
import com.o19s.solr.swan.highlight.spanawarevectorhighlight.SAFragListBuilder;
import com.o19s.solr.swan.highlight.spanawarevectorhighlight.SAFragmentsBuilder;
import com.o19s.solr.swan.highlight.spanawarevectorhighlight.SAScoreOrderFragmentsBuilder;
import com.o19s.solr.swan.highlight.spanawarevectorhighlight.SASimpleFragListBuilder;
import com.o19s.solr.swan.highlight.spanawarevectorhighlight.SASimpleFragmentsBuilder;
import com.o19s.solr.swan.highlight.spanawarevectorhighlight.SASingleFragListBuilder;
import com.o19s.solr.swan.highlight.spanawarevectorhighlight.WordHashFragmentsBuilder;
...
public class SwanHighlighter extends DefaultSolrHighlighter {
	...
	/**
	 * Generates a list of Highlighted query fragments for each item in a list of
	 * documents, or returns null if highlighting is disabled.
	 *
	 ...
	 *
	 * @return NamedList containing a NamedList for each document, which in turns
	 *         contains sets (field, summary) pairs.
	 */
	...
	public NamedList<Object> doHighlighting(DocList docs, Query query, SolrQueryRequest req, String[] defaultFields)
			throws IOException {
		return highLight(docs, query, req, defaultFields);
	}

	private NamedList<Object> highLight(DocList docs, Query query, SolrQueryRequest req, String[] defaultFields)
			throws IOException {
...
//        fieldName = fieldName.trim();
				...
				for (int j = 0; j < docTexts.length; j++) {
					...
					TokenStream ts = TokenSources.getTokenStream(fieldName, tvFields, docTexts[j], analyzer,
							maxCharsToAnalyze - 1);
					...
					try {
						TextFragment[] bestTextFragments = highlighter.getBestTextFragments(tokenStream, docTexts[j],
								mergeContiguousFragments, maxNumFragments);
				...
				// TODO: we can include score and position information in output as snippet
				// attributes
	...
	public NamedList<Object> highlightingFastVector(DocList docs, Query query, SolrQueryRequest req,
		...
		SAFastVectorHighlighter safvh = new SAFastVectorHighlighter(
				...
				params.getBool(HighlightParams.FIELD_MATCH, false), getFragListBuilder(params),
				new WordHashFragmentsBuilder());

		safvh.setPhraseLimit(params.getInt(HighlightParams.PHRASE_LIMIT, Integer.MAX_VALUE));
		SAFieldQuery fieldQuery = new SAFieldQuery(query,  searcher.getIndexReader(),
				params.getBool(HighlightParams.USE_PHRASE_HIGHLIGHTER, true),
				params.getBool(HighlightParams.FIELD_MATCH, false));
		// FieldQuery fieldQuery = safvh.getFieldQuery(query,
	...
	private void doHighlightingByHighlighter(Query query, SolrQueryRequest req, NamedList docSummaries, int docId,
			Document doc, String fieldName, String smMarker, String pmMarker) throws IOException {
		int maxNumFragments = 10000;
		...
		// according to Document javadoc, doc.getValues() never returns null. check
	...
	private void doHighlightingByFastVectorHighlighter(SAFastVectorHighlighter highlighter,
			SAFieldQuery spanAwareFieldQuery, SolrQueryRequest req, NamedList docSummaries, int docId, Document doc,
			String fieldName) throws IOException {
		SolrParams params = req.getParams();
		SolrFragmentsBuilder solrFb = getSolrFragmentsBuilder(fieldName, params);
		String[] snippets = highlighter.getBestFragments(spanAwareFieldQuery, req.getSearcher().getIndexReader(), docId,
				fieldName, params.getFieldInt(fieldName, HighlightParams.FRAGSIZE, 100),
				params.getFieldInt(fieldName, HighlightParams.SNIPPETS, 1),
				getFragListBuilderOverride(fieldName, params), getFragmentsBuilderOverride(fieldName, params),
	...
	protected SAFragmentsBuilder getFragmentsBuilderOverride(String fieldName, SolrParams params) {
		...
		// return getSolrFragmentsBuilderOverride( fieldName, params
		// ).getFragmentsBuilder(params, bs);
	}
	/*
	 * protected SolrFragmentsBuilder getSolrFragmentsBuilderOverride( String
	 * fieldName, SolrParams params ){ String fb = params.getFieldParam( fieldName,
	 * HighlightParams.FRAGMENTS_BUILDER ); SolrFragmentsBuilder solrFb =
	 * fragmentsBuilders.get(fb); if( solrFb == null ){ throw new SolrException(
	 * SolrException.ErrorCode.BAD_REQUEST, "Unknown fragmentsBuilder: " + fb ); }
	 * return solrFb; }
	 */

	/*
	 * protected BoundaryScanner getBoundaryScannerOverride(String fieldName,
	 * SolrParams params){ String bs = params.getFieldParam(fieldName,
	 * HighlightParams.BOUNDARY_SCANNER); SolrBoundaryScanner solrBs =
	 * boundaryScanners.get(bs); if(solrBs == null){ throw new
	 * SolrException(SolrException.ErrorCode.BAD_REQUEST,
	 * "Unknown boundaryScanner: " + bs); } return
	 * solrBs.getBoundaryScanner(fieldName, params); }
	 */
...
/**
 * Orders Tokens in a window first by their startOffset ascending. endOffset is
 * currently ignored. This is meant to work around fickleness in the highlighter
 * only. It can mess up token positions and should not be used for indexing or
 * querying.
 */
 ...
 * 
 * TokenStream bufferedTokenStream = null; OffsetAttribute bufferedOffsetAtt;
 * State bufferedToken; int bufferedStartOffset; int bufferedEndOffset; int
 * startOffset; int endOffset;
 * 
 * public TermOffsetsTokenStream( TokenStream tstream ){ bufferedTokenStream =
 * tstream; bufferedOffsetAtt =
 * bufferedTokenStream.addAttribute(OffsetAttribute.class); startOffset = 0;
 * bufferedToken = null; }
 * 
 * public TokenStream getMultiValuedTokenStream( final int length ){ endOffset =
 * startOffset + length; return new MultiValuedStream(length); }
 * 
 * final class MultiValuedStream extends TokenStream { private final int length;
 * OffsetAttribute offsetAtt = addAttribute(OffsetAttribute.class);
 * 
 * MultiValuedStream(int length) { super(bufferedTokenStream.cloneAttributes());
 * this.length = length; }
 * 
 * @Override public boolean incrementToken() throws IOException { while( true ){
 * if( bufferedToken == null ) { if (!bufferedTokenStream.incrementToken())
 * return false; bufferedToken = bufferedTokenStream.captureState();
 * bufferedStartOffset = bufferedOffsetAtt.startOffset(); bufferedEndOffset =
 * bufferedOffsetAtt.endOffset(); }
 * 
 * if( startOffset <= bufferedStartOffset && bufferedEndOffset <= endOffset ){
 * restoreState(bufferedToken); bufferedToken = null; offsetAtt.setOffset(
 * offsetAtt.startOffset() - startOffset, offsetAtt.endOffset() - startOffset );
 * return true; } else if( bufferedEndOffset > endOffset ){ startOffset +=
 * length + 1; return false; } bufferedToken = null; } } } }
 */