...
/**
 * Syntax precedence taken from LLDiscoveryServer9_0UNIXUsersGuide.pdf p.50 the
 * only difference is that the NOT operator is assumed to have highest
 * precedence instead of near lowest Left associativity assumed for all
 * operators
 * 
 * @author berryman
 *
...
public class SwanParser<V> extends BaseParser<V> {
	...
	public Rule ComExpression() {
		Var<Integer> n = new Var<Integer>();
		return Sequence(AdjProxExpression(),
	...
	public Rule AdjProxExpression() {
		Var<Integer> n = new Var<Integer>();
		return Sequence(NaoExpression(),
				ZeroOrMore(FirstOf(Sequence(PROX(n), NaoExpression(), push(searcher.prox(pop(1), pop(), n.get()))),
	...
	public Rule ClassificationOU() {
		...
		return Sequence(Optional("("), OptionalWhiteSpace(), WordCharNoDashes(), mainClassification.set(match()),
				...
				ZeroOrMore(OptionalWhiteSpace(), ",", OptionalWhiteSpace(),
						FirstOf(PushOuWithRange(mainClassification, subClassification1, subClassification2),
								PushOuWithFloatDigits(mainClassification))),
				OptionalWhiteSpace(),
	...
	public Rule PushDashRange(StringVar mainClassification, StringVar subClassification1,
			StringVar subClassification2) {
		return Sequence(DashRange(subClassification1, subClassification2), push(searcher.classRange(null,
				mainClassification.get(), subClassification1.get(), subClassification2.get())));
	...
	public Rule Clause() {
		...
		return Sequence(FirstOf(Term(n), Phrase(), Parens(), BoundRange(), Range()),
				Optional(Field(n), push(searcher.fieldedExpression(n.get(), pop()))));
	}

	public Rule Field(Var<String> n) {
		return Sequence(FirstOf(DotField(n), BracketField(n)), FirstOf(WhiteSpace(), Test(")"), Test(EOI)));
	}

	public Rule DotField(Var<String> n) {
		return Sequence(".", OneOrMore(WordChar()), n.set(match()), ".");
	}
	...
	public Rule Term(Var<String> n) {
		return Sequence(Sequence(OneOrMore(Char()), ZeroOrMore(TestNot(Field(n)), ".", OneOrMore(Char()))),
				push(searcher.term(match())), FirstOf(// compact this TODO
						WhiteSpace(), Test(")"), Test(EOI), Test("."), Test("[")));
	}

	public Rule Phrase() {
		return Sequence(FirstOf("\"", "'"), OneOrMore(NoneOf("\"'")), push(searcher.phrase(match())),
				FirstOf("\"", "'"), OptionalWhiteSpace());
	}

	public Rule Parens() {
		return Sequence("(", OptionalWhiteSpace(), OuExpression(), push(searcher.wrap(pop())), OptionalWhiteSpace(),
				")", ZeroOrMore("!"), OptionalWhiteSpace());
	}

	public Rule Range() {
		StringVar field = new StringVar();
		StringVar inequality = new StringVar();
		return Sequence("@", OneOrMore(WordChar()), field.set(match()), OptionalWhiteSpace(), RangeOp(),
				inequality.set(match()), OptionalWhiteSpace(), ZeroOrMore("\""), OneOrMore(Char()),
				push(searcher.range(field.get(), inequality.get(), match())), ZeroOrMore("\""),
				FirstOf(WhiteSpace(), Test("("), Test(")"), Test(EOI)));
	}
	...
	public Rule BoundRange() {
		...
		return Sequence("@", OneOrMore(WordChar()), field.set(match()), OptionalWhiteSpace(), RangeOp(),
				inequality1.set(match()), OptionalWhiteSpace(), ZeroOrMore("\""), OneOrMore(Char()),
				match1.set(match()), ZeroOrMore("\""), OptionalWhiteSpace(), RangeOp(), inequality2.set(match()),
				OptionalWhiteSpace(), ZeroOrMore("\""), OneOrMore(Char()),
				push(searcher.boundRange(field.get(), inequality1.get(), match1.get(), inequality2.get(), match())),
				ZeroOrMore("\""), FirstOf(WhiteSpace(), Test("("), Test(")"), Test(EOI)));
	}
	...
	public Rule Char() {
		...
		return FirstOf(WordChar(), AnyOf("/,*?$"));
	}

	public Rule WordChar() {
		return AnyOf("0123456789" + "abcdefghijklmnopqrstuvwxyz" + "ABCDEFGHIJKLMNOPQRSTUVWXYZ" + "-_");
	}
	...
	public Rule FloatChar() {
		return Sequence(OneOrMore(AnyOf("0123456789")), ZeroOrMore(".", OneOrMore(AnyOf("0123456789"))));
	}

	public Rule WordCharNoDashes() {
		return OneOrMore(AnyOf("0123456789." + "abcdefghijklmnopqrstuvwxyz" + "ABCDEFGHIJKLMNOPQRSTUVWXYZ"));
	}
	...
	public Rule ADJ(Var<Integer> n) {
		return Sequence(IgnoreCase("ADJ"), OptionalDigits(), n.set(Integer.parseInt(matchOrDefault("1"))),
				CharAfterOp());
	}
	...
	public Rule CharAfterOp() {
		// TODO there are a couple of these rules that contain EOI - I feel like that's
		// an anti pattern
		return FirstOf(WhiteSpace(), Test("("), Test(EOI));
	}
